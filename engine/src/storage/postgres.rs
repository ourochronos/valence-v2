...
impl TripleStore for PgStore {
    ...
    async fn insert_triple(&self, triple: Triple) -> Result<TripleId> {
        sqlx::query(
            r#"
            INSERT INTO triples (id, subject_id, predicate, object_id, origin_did, timestamp, signature, base_weight, local_weight, access_count, last_accessed)
        ...
        .bind(triple.object)
        .bind(&triple.origin_did)
        .bind(triple.timestamp)
        .bind(&triple.signature)
        .bind(triple.base_weight)
        .bind(triple.local_weight)
    ...
    async fn get_triple(&self, id: TripleId) -> Result<Option<Triple>> {
        let row = sqlx::query(
            r#"
            SELECT id, subject_id, predicate, object_id, origin_did, timestamp, signature, base_weight, local_weight, access_count, last_accessed
            FROM triples
            WHERE id = $1
            "#
        )
        ...
        Ok(row.map(|r| Triple {
            ...
            origin_did: r.get("origin_did"),
            timestamp: r.get("timestamp"),
            signature: r.get("signature"),
            base_weight: r.get("base_weight"),
            local_weight: r.get("local_weight"),
            access_count: r.get::<i64, _>("access_count") as u64,
            last_accessed: r.get("last_accessed"),
        }))
    }
    ...
    async fn update_triple(&self, triple: Triple) -> Result<()> {
        sqlx::query(
            r#"
            UPDATE triples
            SET subject_id = $2, predicate = $3, object_id = $4, origin_did = $5,
                base_weight = $6, local_weight = $7, access_count = $8, last_accessed = $9
            WHERE id = $1
            "#
        )
        .bind(triple.id)
    ...
    async fn query_triples(&self, pattern: TriplePattern) -> Result<Vec<Triple>> {
        let mut query = String::from(
            "SELECT id, subject_id, predicate, object_id, origin_did, timestamp, signature, base_weight, local_weight, access_count, last_accessed FROM triples WHERE 1=1"
        );
    ...
    async fn neighbors(&self, node_id: NodeId, depth: u32) -> Result<Vec<Triple>> {
        ...
        let query = r#"
            WITH RECURSIVE neighbor_triples AS (
                SELECT t.id, t.subject_id, t.predicate, t.object_id, t.origin_did, t.timestamp, t.signature,
                       t.base_weight, t.local_weight, t.access_count, t.last_accessed, 1 as depth
                FROM triples t
                ...
                SELECT t.id, t.subject_id, t.predicate, t.object_id, t.origin_did, t.timestamp, t.signature,
                       t.base_weight, t.local_weight, t.access_count, t.last_accessed, nt.depth + 1
                FROM triples t
            ...
            SELECT DISTINCT id, subject_id, predicate, object_id, origin_did, timestamp, signature, base_weight, local_weight, access_count, last_accessed
    ...
    async fn decay(&self, factor: f64, min_weight: f64) -> Result<u64> {
        let result = sqlx::query(
            r#"
            UPDATE triples
            SET local_weight = GREATEST(local_weight * $1, $2)
            WHERE local_weight >= $2
            "#
        )
        .bind(factor)
        .bind(min_weight)
    ...
    async fn evict_below_weight(&self, threshold: f64) -> Result<u64> {
        let result = sqlx::query("DELETE FROM triples WHERE local_weight < $1")
            .bind(threshold)
            .execute(&self.pool)
            .await
